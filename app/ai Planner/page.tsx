"use client";

import Link from "next/link";
import { useMemo, useState } from "react";

export default function AIPlannerPage() {
  const [city, setCity] = useState("Ahmedabad");
  const [eventType, setEventType] = useState("Wedding");
  const [guests, setGuests] = useState(250);
  const [budget, setBudget] = useState(800000);
  const [style, setStyle] = useState("Luxury Royal");
  const [days, setDays] = useState(2);

  const plan = useMemo(() => {
    // Smart-ish rules (deploy-safe, no API)
    const perGuest = budget / Math.max(guests, 1);
    const tier = perGuest > 4500 ? "Ultra Luxury" : perGuest > 2500 ? "Luxury" : perGuest > 1500 ? "Premium" : "Budget Smart";

    const split = [
      ["Venue + Catering", Math.round(budget * 0.45)],
      ["Decor + Lighting", Math.round(budget * 0.18)],
      ["Photo + Video", Math.round(budget * 0.08)],
      ["Makeup + Styling", Math.round(budget * 0.06)],
      ["Entertainment", Math.round(budget * 0.06)],
      ["Logistics", Math.round(budget * 0.05)],
      ["Gifts + Misc", Math.round(budget * 0.07)],
      ["Buffer", Math.max(0, budget - Math.round(budget * 0.95))],
    ];

    const timeline =
      days === 1
        ? [
            "Welcome & Guest Check-in",
            "Main Ceremony / Function",
            "Dinner Service",
            "After-party / DJ",
          ]
        : [
            "Day 1: Welcome + Mehendi / Sangeet",
            "Day 2: Wedding Ceremony + Reception",
          ];

    const checklist = [
      "Finalize guest count & budget range",
      "Shortlist 3 venues (capacity + location)",
      "Lock date & venue booking",
      "Choose decor moodboard & color palette",
      "Book photo/video & makeup",
      "Finalize catering menu & tasting",
      "Create function-wise timeline",
      "Confirm logistics: transport, entry, parking, rooms",
      "Final rehearsal + event-day coordination plan",
    ];

    const nextSteps = [
      `Message Eventura on WhatsApp with: ${city}, ${eventType}, ${guests} guests, ₹${budget.toLocaleString()} budget, style: ${style}`,
      "We will share a venue shortlist + decor direction + initial quote",
      "Confirm your date and we start vendor locking",
    ];

    return { tier, split, timeline, checklist, nextSteps };
  }, [city, eventType, guests, budget, style, days]);

  const waText = encodeURIComponent(
    `Hi Eventura! AI Planner Summary:\nCity: ${city}\nEvent: ${eventType}\nGuests: ${guests}\nDays: ${days}\nBudget: ₹${budget}\nStyle: ${style}\nPlease share a venue shortlist + quote.`
  );

  return (
    <div style={S.page}>
      <header style={S.header}>
        <div style={S.headerInner}>
          <Link href="/" style={S.brand}>Eventura</Link>
          <Link href="/book" style={{ ...S.btn, ...S.btnSolid }}>Book</Link>
        </div>
      </header>

      <main style={{ padding: 16 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <h1 style={{ marginBottom: 6 }}>AI Event Planner</h1>
          <p style={{ opacity: 0.75, lineHeight: 1.7 }}>
            Get an instant plan: budget split, checklist & timeline — then message us for a real quote.
          </p>

          <div style={S.grid2}>
            <div style={S.panel}>
              <div style={S.panelTitle}>Your inputs</div>

              <label style={S.label}>City</label>
              <select value={city} onChange={(e) => setCity(e.target.value)} style={S.input}>
                {["Ahmedabad","Surat","Vadodara","Rajkot","Gandhinagar"].map((c) => <option key={c}>{c}</option>)}
              </select>

              <label style={S.label}>Event type</label>
              <select value={eventType} onChange={(e) => setEventType(e.target.value)} style={S.input}>
                {["Wedding","Engagement","Reception","Corporate","Birthday"].map((t) => <option key={t}>{t}</option>)}
              </select>

              <label style={S.label}>Guests</label>
              <input type="number" value={guests} onChange={(e) => setGuests(Number(e.target.value || 0))} style={S.input} />

              <label style={S.label}>Budget (₹)</label>
              <input type="number" value={budget} onChange={(e) => setBudget(Number(e.target.value || 0))} style={S.input} />

              <label style={S.label}>Event days</label>
              <select value={days} onChange={(e) => setDays(Number(e.target.value))} style={S.input}>
                <option value={1}>1 Day</option>
                <option value={2}>2 Days</option>
                <option value={3}>3 Days</option>
              </select>

              <label style={S.label}>Style</label>
              <select value={style} onChange={(e) => setStyle(e.target.value)} style={S.input}>
                {["Luxury Royal","Modern Minimal","Floral Elegant","Luxe Glam","Traditional Gujarati"].map((s) => <option key={s}>{s}</option>)}
              </select>

              <a href={`https://wa.me/917622856869?text=${waText}`} target="_blank" rel="noreferrer" style={{ ...S.btn, ...S.btnGold, width: "100%", marginTop: 10 }}>
                Send plan to WhatsApp
              </a>
            </div>

            <div style={S.panel}>
              <div style={S.panelTitle}>Your AI plan</div>

              <div style={S.pill}>Planning tier: <b>{plan.tier}</b></div>

              <h3>Budget split</h3>
              <ul style={S.ul}>
                {plan.split.map(([k, v]) => (
                  <li key={k}><b>{k}</b>: ₹{Number(v).toLocaleString()}</li>
                ))}
              </ul>

              <h3>Timeline</h3>
              <ul style={S.ul}>
                {plan.timeline.map((t) => <li key={t}>{t}</li>)}
              </ul>

              <h3>Checklist</h3>
              <ul style={S.ul}>
                {plan.checklist.map((t) => <li key={t}>{t}</li>)}
              </ul>

              <h3>Next steps</h3>
              <ol style={S.ol}>
                {plan.nextSteps.map((t) => <li key={t}>{t}</li>)}
              </ol>
            </div>
          </div>
        </div>
      </main>
    </div>
  );
}

const S: Record<string, React.CSSProperties> = {
  page: { minHeight: "100vh", background: "#07070a", color: "white" },
  header: { borderBottom: "1px solid rgba(255,255,255,.10)" },
  headerInner: { maxWidth: 1100, margin: "0 auto", padding: 16, display: "flex", justifyContent: "space-between" },
  brand: { color: "white", textDecoration: "none", fontWeight: 950 },
  grid2: { marginTop: 12, display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr" },
  panel: { padding: 16, borderRadius: 18, border: "1px solid rgba(255,255,255,.12)", background: "rgba(255,255,255,.05)" },
  panelTitle: { fontWeight: 980, marginBottom: 10 },
  label: { display: "block", marginTop: 10, opacity: 0.7, fontSize: 12, letterSpacing: "0.12em", textTransform: "uppercase" },
  input: { width: "100%", height: 44, borderRadius: 12, border: "1px solid rgba(255,255,255,.18)", background: "rgba(0,0,0,.35)", color: "white", padding: "0 12px" },
  btn: { padding: "10px 14px", borderRadius: 14, border: "1px solid rgba(255,255,255,.18)", color: "white", textDecoration: "none", fontWeight: 900, display: "inline-flex", justifyContent: "center", alignItems: "center" },
  btnSolid: { background: "white", color: "black" },
  btnGold: { background: "linear-gradient(180deg, rgba(201,162,74,.95), rgba(201,162,74,.65))", color: "#120f08", borderColor: "rgba(201,162,74,.55)" },
  pill: { display: "inline-block", padding: "8px 10px", borderRadius: 999, border: "1px solid rgba(201,162,74,.35)", background: "rgba(201,162,74,.12)", color: "rgba(255,255,255,.92)" },
  ul: { opacity: 0.82, lineHeight: 1.8 },
  ol: { opacity: 0.82, lineHeight: 1.8 },
};
